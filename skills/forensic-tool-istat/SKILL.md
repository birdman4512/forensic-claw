---
name: forensic-tool-istat
description: Use istat in Forensic Claw for lawful DFIR, cyber security, evidence triage, intrusion analysis, and case-backed forensic workflows. Inspect inode/metadata records in filesystem images.
---
...
```bash
tools/run-forensic-tool.sh istat [args...]
```

Use after `fls` identifies inode references. Example: `tools/run-forensic-tool.sh istat -o <offset> image.dd <inode>`.
