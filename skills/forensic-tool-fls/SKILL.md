---
name: forensic-tool-fls
description: Use fls in Forensic Claw for lawful DFIR, cyber security, evidence triage, intrusion analysis, and case-backed forensic workflows. List files and directories from disk images using Sleuth Kit.
---
...
```bash
tools/run-forensic-tool.sh fls [args...]
```

Use with `mmls` offsets where needed. Example: `tools/run-forensic-tool.sh fls -r -o <sector_offset> image.dd`.
